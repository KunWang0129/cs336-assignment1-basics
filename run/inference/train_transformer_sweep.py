...
def Tokenizer_dataset(
    ...
    print(f"Starting bpe tokenization")
    start_time = time.time()
    ...
    # Process training set (stream encoding)
    ...
    # Process validation set (stream encoding)
...
def model_pipeline(hyperparameters=None):
    with wandb.init(config=hyperparameters):
        ...
        while iteration <= total_iters:
            input_train, target_train = data_loader(
            ...
            if iteration % val_interval == 0 or iteration == total_iters:
                ...
                if val_loss < best_val_loss:
                    ...
                    save_checkpoint(model, optimizer, iteration, MODULE_PATH / ckpt_name)
...
if __name__ == '__main__':
    ...
    trainencode_name = 'TinyStoriesV2-GPT4_tokens.bin'
    validencode_name = 'TinyStoriesV2-GPT4_tokens.bin'
    ckpt_name = 'TinyStoriesV2-GPT4checkpoint.pt'
    vocab_size = 10000
    batch_size = 128
    context_length = 256
    d_model = 512
    ...
    valid_dataset = np.memmap(DATA_A_PATH / validencode_name, dtype=np.uint16, mode="r")
    start_iter = 1
    total_iters = 1200 # 10000
    ...
    sweep_id = wandb.sweep(sweep_config, project="cs336_assignment1")
